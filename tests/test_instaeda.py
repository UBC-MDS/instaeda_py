...
def test_plot_basic_distributions(input_dataframe):
    ...
    with pytest.raises(KeyError) as exc_info:
        instaeda.plot_basic_distributions(input_dataframe,
                                          cols=['invalid column name'])
    assert 'are in the [columns]' in str(exc_info.value)

    with pytest.raises(KeyError) as exc_info:
        instaeda.plot_basic_distributions(input_dataframe,
                                          include='unsupported datatype')
    ...
    assert list(dict_plots.keys()) == ['bill_length_mm',
                                       'bill_depth_mm',
                                       'flipper_length_mm',
                                       'body_mass_g',
                                       'year',
...
def test_divide_and_fill(input_dataframe):
    ...
    try:
        ...
        raise Exception('''
            Expected an Exception, but none were raised. Not a dataframe.
            ''')
    ...
    assert_frame_equal(instaeda.divide_and_fill(not_na_dataframe),
                       not_na_dataframe, check_dtype=False)

    # Test that the output for na numerical column
    assert isinstance(instaeda.divide_and_fill(na_numerical_dataframe),
                      pd.DataFrame)
    assert_frame_equal(instaeda.divide_and_fill(na_numerical_dataframe),
                       not_na_dataframe, check_dtype=False)

    # Test that the output for na categorical column
    assert isinstance(instaeda.divide_and_fill(na_categorical_dataframe,
                                               cols=['col_3'],
                                               strategy='constant',
                                               fill_value='b'),
                      pd.DataFrame)
    assert_frame_equal(instaeda.divide_and_fill(na_categorical_dataframe,
                                                cols=['col_3'],
                                                strategy='constant',
                                                fill_value='b'),
                       not_na_dataframe, check_dtype=False)
    ...
    try:
        instaeda.divide_and_fill(na_numerical_dataframe,
                                 cols=["col_1", "col_3"])
    ...
    try:
        ...
        raise Exception('''
            Expected an Exception, but none were raised. Strategy.
            ''')
...
def test_plot_corr(input_dataframe):
    ...
    with pytest.warns(UserWarning) as exc_info:
        instaeda.plot_corr(input_dataframe,
                           colour_palette="non supported color palette")
...
def test_plot_intro(input_dataframe):
    ...
    assert isinstance(theme_config, str)
